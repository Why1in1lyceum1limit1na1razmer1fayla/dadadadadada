...
def proigrish():
    top10 = open("files/top10.txt", 'r')
    sp_top = list(map(int, top10.read().split('\n')))
    top10.close()
    print(sp_top)
    global matrix
    global ochki
    ...
    spawn()
    if len(sp_top) < 10:
        top10 = open("files/top10.txt", 'w')
        sp_top.append(ochki)
        sp_top.sort()
        stroka = ''
        sp_top.sort(reverse=True)
        for i in range(len(sp_top) - 1):
            stroka += str(sp_top[i]) + '\n'
            print(sp_top[i])
        print(sp_top[-1])
        stroka += str(sp_top[-1])
